...
import os
import sub2vec_v2.randonWalk as rw
import sub2vec_v2.doc2vec as d2v
...
class Sub2Vec(AbstractController):
    ...
    def setup(self):
        self.randomWalk_length = getConfig().eval(self.__class__.__name__, 'random_walk_length')
        self.random_walk_num = getConfig().eval(self.__class__.__name__, 'random_walk_number')
        self.subGraphs_directory_path = getConfig().eval(self.__class__.__name__, 'subGraphs_directory_path')
        self.random_walk_directory_path_output = getConfig().eval(self.__class__.__name__,
                                                                  'random_walk_directory_path_output')
        self.classifier_files_directory = getConfig().eval(self.__class__.__name__, 'classifier_files_directory')
        self.statistics_output_path = getConfig().eval(self.__class__.__name__, 'statistics_output_path')
        ...
        self.rw_args = json.loads(getConfig().eval(self.__class__.__name__, 'randomwalk_args'))
        self.rw_extensions = getConfig().eval(self.__class__.__name__, 'rw_extensions').split(",")
        self.doc2vec_args = json.loads(getConfig().eval(self.__class__.__name__, 'doc2vec_args'))
        ...
        for filename in os.listdir(self.subGraphs_directory_path):
            try:
                self.subGraphs_list.append(
                    (filename, (nx.read_gpickle(os.path.join(self.subGraphs_directory_path, filename)))))
            except:
                continue
    ...
    def randomWalk(self):
        self.main_graph = nx.read_gml(
            self.getPath(relative_path=getConfig().eval(self.__class__.__name__, 'main_graph_path')))
        random_walk_object = rw.RandomWalk(threshold=self.randomWalk_length,
                                           number_of_graphs=self.random_walk_num, args=self.rw_args,
                                           extensions=self.rw_extensions, main_graph=self.main_graph)
    ...
    def chooseBestSubgraphs(self):
        ...
        self.bestPositives()
        self.bestNegatives()
        self.rw_list_of_graphs_train = self.rw_list_of_graphs_train_negative[
                                       :100] + self.rw_list_of_graphs_train_positive[:100]
    ...
    def statistics(self):
        columns = ["correlation threshold", "rw length", "rw number of times", "extensions",
                   "Main Graph: number of nodes", "Main Graph: number of edges", "number of subgraphs",
    ...
    def getImprovments(self):
        extensions = [
            str(self.config_parser.eval('GraphCreator', 'adj_matrix_extensions')).replace('}', '').replace('{',
                                                                                                           '').replace(
                ':', '=').replace("'", ""),
